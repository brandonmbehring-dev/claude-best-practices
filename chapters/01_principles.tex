...
in your terminal.\sourceurl{https://docs.anthropic.com/en/docs/claude-code/overview}}

Claude Code runs an \textbf{agent loop}: it reads your prompt, reasons about
...
\begin{description}
  \item[Context window] Everything Claude knows about your task---your prompt,
  ...
  \item[Tool use] Claude does not type code into a file. It calls tools:
...
and 2-3 options for resolution. Never work around errors silently.
\end{minted}
\end{beforeafter}
...
% -------------------------------------------------------------------
\begin{trybox}
Pick one function in your current project---ideally one Claude will read often.
Audit it against the four principles:
\begin{enumerate}
  \item Does it fail silently anywhere? (catches without re-raising or logging)
  \item Is it under 30 lines? If not, can it be split?
  \item Does it mutate its inputs? If so, is that explicit in the name?
