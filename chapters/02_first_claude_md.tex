...
\official{The \code{CLAUDE.md} file is the single most important configuration artifact.\sourceurl{https://code.claude.com/docs/en/best-practices}}\index{CLAUDE.md}
...
\begin{enumerate}
  \item \textbf{Managed policy} (\code{managed-settings.json} CLAUDE.md) --- IT-deployed,
    highest priority (enterprise only)
...
The fastest path to a useful CLAUDE.md:\index{CLAUDE.md!init command@\texttt{/init} command}
...
\convergence{Include what Claude cannot infer. Exclude what it can.\sourceurl{https://code.claude.com/docs/en/best-practices}}\index{CLAUDE.md!what to include}
...
\begin{tipbox}[Write Positive Instructions]
Replace negative constraints with positive alternatives.
\emph{``Never use raw SQL''} leaves Claude uncertain about what to use.
\emph{``Always use the ORM query builder in \code{src/db/queries.py}''} tells
Claude exactly what to do instead.
Positive instructions are clearer, more actionable, and more reliably followed.
\end{tipbox}
...
Before Claude touches any code, protect your secrets.\index{CLAUDE.md!deny rules}
