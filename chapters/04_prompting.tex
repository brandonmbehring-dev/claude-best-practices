...
Current code is in src/payments/webhook_handler.py.
</context>

<task>
Add idempotency checking using the Stripe event ID.
Store processed event IDs in Redis with 24h TTL.
</task>

<constraints>
- Must not break existing webhook processing
- Redis connection is available via app.redis
...
New tests must cover: duplicate event, expired event, new event.
</verification>
\end{minted}
...
\begin{minted}{text}
Look at @src/payments/webhook_handler.py and @tests/payments/test_webhooks.py.
The handler processes events but has no idempotency check.
