...
\after
\begin{minted}{text}
I see a TypeError on line 47 of processor.py:
None + int. Three hypotheses:

1. get_score() returns None when the user
   has no history
2. The database query returns NULL for
   inactive accounts
3. A race condition: score is deleted between
   check and use

For hypothesis 1: write a test that calls
  get_score() with a new user (no history).
For hypothesis 2: write a test with an
  inactive account fixture.
For hypothesis 3: check if get_score() uses
  a cache that could be invalidated.
...
\begin{trybox}
Pick a recent bug---ideally one that took more than 10 minutes to resolve.
...
\after
``Here is my schema. I designed it assuming: (1) reads outnumber
writes 10:1, (2) we never query across tenants, (3) a single Postgres
instance is sufficient. Which assumption is most likely wrong
in 12 months, and what breaks when it does?''
\end{beforeafter}
...
# ADR-007: Event-Driven Payment Processing

## Context
Payment processing is synchronous.
Under load, response times exceed 2s SLA.

## Decision
Switch to event-driven via RabbitMQ.

## Alternatives Considered
1. Async HTTP (rejected: still coupled)
2. Redis pub/sub (rejected: no persistence)
3. Kafka (rejected: overkill at current scale)

## Consequences
- Eventual consistency (not immediate)
- New failure mode: message queue backlog
- Need dead-letter queue for failed events

## Assumptions to Revisit
- Message volume < 1000/min
- Single consumer is sufficient
- RabbitMQ managed service available
\end{minted}
...
\begin{enumerate}
  \item \textbf{Easiest}: Add a docstring to one function Claude reads often.
