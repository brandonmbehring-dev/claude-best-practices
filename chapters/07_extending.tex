...
\official{Skills combine domain knowledge with executable workflows.\sourceurl{https://code.claude.com/docs/en/skills}}
...
Use \code{/context} to check for excluded skills.\sourceurl{https://code.claude.com/docs/en/skills}}
...
\toprule
\textbf{Event} & \textbf{Matcher Target} & \textbf{Use Case} \\
\midrule
\code{SessionStart} & How session started & Load context, run health check \\
\code{InstructionsLoaded} & (no matcher) & When CLAUDE.md or rules files are loaded \\
...
\code{Notification} & Notification type & Route to Slack, email, or dashboard \\
\code{Elicitation} & (no matcher) & MCP server requests structured user input \\
\code{ElicitationResult} & (no matcher) & User responds to an elicitation dialog \\
\code{SubagentStart} & Agent type & Initialize subagent-specific config \\
\code{SubagentStop} & Agent type & Process subagent results \\
...
\code{ConfigChange} & Config source & React to settings file changes \\
\code{WorktreeCreate} & (no matcher) & Custom worktree setup logic \\
\code{WorktreeRemove} & (no matcher) & Custom worktree cleanup logic \\
\code{PreCompact} & Trigger type & Save state before context compaction \\
\code{PostCompact} & (no matcher) & Process results after compaction completes \\
\code{SessionEnd} & Why session ended & Save state, update logs \\
\bottomrule
\end{tabular}
\end{fullwidth}
...
Hooks support four handler types. Command timeout is 10 minutes (increased
...
external services, databases, APIs, and custom tools.\sourceurl{https://code.claude.com/docs/en/mcp}}
...
\official{Permission rules can use wildcards for MCP tool scoping:\sourceurl{https://code.claude.com/docs/en/permissions}}
...
\official{Plugins are pre-packaged extensions from the community and Anthropic.\sourceurl{https://code.claude.com/docs/en/plugins}}
...
\begin{description}
  \item[Code intelligence plugins] \official{For typed languages, install a code
    intelligence plugin immediately.\sourceurl{https://code.claude.com/docs/en/common-workflows}}
    These give Claude precise ``go to definition'' and ``find references''
    capabilities, plus automatic error detection after edits.
    Supported languages include TypeScript, Rust, Go, Java, and Python (via Pyright).
    The improvement in code navigation accuracy is substantial.
...
within defined boundaries.\sourceurl{https://code.claude.com/docs/en/sandboxing}}
