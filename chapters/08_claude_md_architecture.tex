...
\index{CLAUDE.md!architecture}\index{CLAUDE.md!hub-and-spoke}
...
\index{CLAUDE.md!imports@\texttt{@}-imports}
...
\subsection{Advanced Rules Patterns}

\official{Additional rules capabilities:\sourceurl{https://code.claude.com/docs/en/memory}}

\begin{description}
  \item[User-level rules] Place \code{.md} files in
    \code{\textasciitilde/.claude/rules/} for personal rules that apply across all projects
    (e.g., communication preferences, personal tooling conventions).\index{CLAUDE.md!user rules}
  \item[Brace expansion] Path patterns support brace expansion:
    \code{paths: \{src,tests\}/**/*.py} matches files in both directories.
  \item[Symlinks] Symlink shared rule files across projects instead of duplicating.
    Useful when hub-and-spoke \code{@imports} are not sufficient.
  \item[\code{claudeMdExcludes}] In \code{settings.json}, list directory patterns
    where CLAUDE.md files should \emph{not} be loaded.\index{CLAUDE.md!claudeMdExcludes} Essential for monorepos
    where dependency directories contain their own CLAUDE.md files:
\end{description}
...
Claude Code uses a 5-level settings hierarchy.\index{CLAUDE.md!loading order}
...
\marginwarning{Anthropic recommends under
\textbf{200 lines per file} for adherence\sourceurl{https://code.claude.com/docs/en/memory}
and under \textbf{$\sim$500 lines total} for
cost.\sourceurl{https://code.claude.com/docs/en/costs}
Our recommendation: $\sim$300 lines per core
file as a practitioner middle ground.}

\marginnote[Tip]{Offload to skills, rules files, and \code{@imports} to stay
within the 200-line per-file target while keeping total effective config
under $\sim$500 lines.}

\practitioner{When CLAUDE.md grows long:}\index{CLAUDE.md!line count (200/500)}
...
or \code{--output-style} flag.\sourceurl{https://code.claude.com/docs/en/best-practices}}

\official{Output styles customize how Claude structures its responses:
tone, verbosity, formatting conventions, and problem-solving protocols.\sourceurl{https://code.claude.com/docs/en/best-practices}}
\index{output styles}
