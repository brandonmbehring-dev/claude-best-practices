...
\official{Subagents are purpose-built Claude instances that run in isolated context.\sourceurl{https://code.claude.com/docs/en/sub-agents}}
\index{subagents}They receive a task, execute it with their own tool set, and return results
...
\subsection{Subagent Frontmatter Reference}
\index{subagents!frontmatter}
...
\midrule
\code{name} & Display name. Lowercase, hyphens. \\
\code{description} & What the agent does. Used for auto-dispatch. \\
\code{tools} & Tools permitted (note: \emph{not} \code{allowed-tools}). \\
\code{model} & Override model (\code{sonnet}, \code{opus}, \code{haiku}). \\
\code{isolation} & Set to \code{worktree} for isolated file system (\cref{sec:worktrees}). \\
\code{memory} & Persistent memory scope: \code{user}, \code{project}, or \code{local}. \\
\code{background} & Run in background; parent notified on completion. \\
\code{skills} & List of skills available to the subagent. \\
\code{mcpServers} & MCP servers the subagent can access. \\
\bottomrule
\end{tabular}
\end{fullwidth}

\marginnote[Official]{Manage subagent configurations with the \code{/agents}
command.\sourceurl{https://code.claude.com/docs/en/sub-agents}}

\subsection{Built-In Subagents}

Claude Code ships with several built-in subagent types\index{subagents!built-in types} that can be invoked
...
\subsection{Continuing Subagent Work}

\official{To continue or resume a stopped subagent, use \code{SendMessage}
with the agent's ID or name.\sourceurl{https://code.claude.com/docs/en/sub-agents}}
\index{subagents!SendMessage}
The subagent resumes with its full context preserved.
Transcripts persist in \code{\textasciitilde/.claude/projects/\{project\}/\{session\}/subagents/}.

\practitioner{In practice, ask Claude to ``continue the review agent'' or
``resume agent X''---Claude translates this to \code{SendMessage} internally.
Each new \code{Agent} invocation starts fresh; \code{SendMessage} is
the only way to preserve prior context.}

% -------------------------------------------------------------------
\section{When to Delegate}
\label{sec:delegation-decision}

\index{delegation!when to delegate}\begin{decisionbox}[Delegate or Do Directly?]
\textbf{Delegate when:}
\begin{itemize}
...
% -------------------------------------------------------------------
\section{Designing Effective Agent Tasks}
\label{sec:agent-design}

\marginnote[Tip]{A well-designed agent task is
self-contained: clear goal,
bounded scope, verifiable output.}

Knowing \emph{when} to delegate is necessary but not sufficient.
The quality of the delegation determines the quality of the result.

\subsection*{The Three Properties of a Good Agent Task}

\begin{enumerate}
  \item \textbf{Self-contained.} The agent must be able to complete
    the task without asking the parent for clarification. If the agent
    needs context from your session, either pass it explicitly in the
    prompt or do the task yourself.

  \item \textbf{Verifiable.} The agent's output must be checkable.
    ``Review this code'' produces opinions. ``Run the test suite and
    report failures with file:line references'' produces verifiable facts.
    Prefer tasks with concrete success criteria.

  \item \textbf{Bounded.} An agent exploring ``everything wrong with
    this codebase'' will consume its entire context window and return
    an unfocused report. Constrain scope: ``Review \code{src/auth/}
    for SQL injection vulnerabilities.''
\end{enumerate}

\subsection*{When Parallel Execution Helps (and When It Doesn't)}

Parallel agents save wall-clock time only when tasks are
\emph{truly independent}. The test:

\begin{itemize}
  \item Can Agent B start without knowing Agent A's result?
    $\rightarrow$ \textbf{Parallel.}
  \item Does Agent B's work depend on Agent A's output?
    $\rightarrow$ \textbf{Sequential.} Run A first, feed results to B.
  \item Do both agents modify the same files?
    $\rightarrow$ \textbf{Use worktrees} or make one read-only.
\end{itemize}

\noindent Parallel execution has overhead: each agent loads its own
context, reads files independently, and may duplicate exploration.
For tasks under 2 minutes, the overhead exceeds the savings.

\practitioner{The sweet spot is 2--4 agents on tasks that each take
5--15 minutes. Below that, sequential is faster. Above 4 agents,
coordination overhead grows and results quality degrades because
you cannot review them all carefully.}

\subsection*{Designing Agent Prompts}

\begin{enumerate}
  \item \textbf{State the role}, not just the task.
    ``You are a security specialist'' focuses attention differently
    than ``review this code.''
  \item \textbf{Specify the output format.}
    ``Report findings as: file:line | severity | description''
    produces structured, comparable results across agents.
  \item \textbf{Tell the agent what to ignore.}
    ``Do not report style issues---focus only on correctness''
    prevents noise that obscures real findings.
  \item \textbf{Restrict tools to what's needed.}
    A review agent needs \code{Read, Grep, Glob}---not \code{Edit}
    or \code{Bash}. Fewer tools = fewer distractions.
\end{enumerate}

\begin{keyinsight}
The most common delegation failure is vague tasking.
``Review this PR'' produces generic feedback.
``Review \code{src/payments/} for: (1) race conditions in concurrent
checkout, (2) amount calculations with floating-point precision issues,
(3) missing error handling on payment gateway timeouts'' produces
actionable findings. Specificity in the prompt determines specificity
in the output.
\end{keyinsight}
...
Worktrees\index{worktrees} solve the fundamental problem of parallel file modifications:
...
\code{isolation} frontmatter key:\sourceurl{https://code.claude.com/docs/en/sub-agents}}
\index{worktrees!isolation}
...
\index{Writer/Reviewer pattern}

\official{Use separate sessions to avoid confirmation bias:\sourceurl{https://code.claude.com/docs/en/best-practices}}
...
and team lead orchestration.\sourceurl{https://code.claude.com/docs/en/agent-teams}}
\index{Agent Teams}
...
Agent Teams require \code{CLAUDE\_CODE\_EXPERIMENTAL\_AGENT\_TEAMS=1}.
The API and behavior may change. Use for exploration, not production workflows.
\end{warnbox}

% -------------------------------------------------------------------
\section{The \code{/batch} Skill: Parallel Codebase Changes}
\label{sec:batch}

\marginnote[Official]{\code{/batch} is a bundled skill, not a built-in command.
Claude orchestrates the work using its
tools.\sourceurl{https://code.claude.com/docs/en/skills}}

\official{\code{/batch} orchestrates large-scale changes across a codebase
in parallel.\sourceurl{https://code.claude.com/docs/en/skills}}
\index{batch@\texttt{/batch}}
It researches the codebase, decomposes the work into 5--30 independent units,
presents a plan for approval, then spawns one background agent per unit
in an isolated git worktree. Each agent implements its unit, runs tests,
...
\begin{decisionbox}[\code{/batch} vs.\ Fan-Out Script]
\textbf{\code{/batch}}: Claude researches, decomposes, and coordinates.
Best for: complex changes where decomposition requires understanding the codebase.

\textbf{Fan-out} (\code{for f in ...; claude -p ...}): you define the units.
Best for: simple, repetitive changes where you already know the file list
(\cref{sec:headless}).
\end{decisionbox}

\margincost{\code{/batch} spawns N agents $\times$ per-agent context.
A 20-unit batch costs $\sim$20$\times$ a single session.
Test on 2--3 units before scaling---the initial plan review is your
checkpoint.}
...
\begin{itemize}
  ...
  \item Subagent frontmatter: \code{tools}, \code{isolation}, \code{memory}, \code{background}, \code{skills}, \code{mcpServers}
  \item Continue a subagent: \code{SendMessage} (not a new \code{Agent} invocation)
  \item Manage configurations: \code{/agents} command
  ...
  \item \code{/batch}: Claude decomposes, spawns parallel agents in worktrees, each opens a PR
  \item Agent Teams: experimental (\code{CLAUDE\_CODE\_EXPERIMENTAL\_AGENT\_TEAMS=1})
\end{itemize}
