...
\begin{enumerate}
  ...
  \item \textbf{Add one hook}: \code{PostToolUse} matching \code{Edit|Write}
    $\rightarrow$ auto-format. This prevents style drift from the first file onward.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Configure PreToolUse hook} matching \code{Bash} $\rightarrow$
    gate \code{git commit} on passing tests.
...
\begin{enumerate}
  ...
  \item \textbf{Transition from exploration to development phase}
    (\cref{sec:phases}). Add explicit graduation criteria to CLAUDE.md.
...
\index{projects!brownfield}

\marginnote[Official]{Use Plan Mode first---let Claude read and understand
the codebase without modifying anything.\sourceurl{https://code.claude.com/docs/en/overview}}

\subsection{Start with Understanding, Not Action}

\official{Before any changes, use Plan Mode (Shift+Tab) to let Claude explore.\sourceurl{https://code.claude.com/docs/en/overview}}
Ask:
\begin{itemize}
...
\official{Document existing conventions in CLAUDE.md, not aspirational ones.\sourceurl{https://code.claude.com/docs/en/best-practices}}
...
\index{projects!refactoring cycle}
...
\practitioner{For data pipelines, a golden master captures the exact output of the original
...
\begin{decisionbox}[Characterization Tests vs.\ Golden Masters]
\textbf{Characterization tests}: for individual functions.
``Document exactly what this function does today.''
Best when: refactoring a function's internals without changing its API.

\textbf{Golden masters}: for pipelines and multi-step workflows.
``Capture the exact output of this pipeline on sample data.''
Best when: migrating pipeline code (e.g., pandas to polars, notebook to module).

\textbf{Both}: during pipeline migration.
Characterization tests verify each function; golden masters verify
the composed pipeline produces identical output.
\end{decisionbox}
...
\centering
\begin{tikzpicture}[
  phasebox/.style={draw=WarmBlue, fill=WarmBlue!8, rounded corners=6pt,
  ...
  arr/.style={->, very thick, WarmBlue!60},
  >=Stealth
]
  ...
  \node[font=\footnotesize\itshape, text=WarmGreen!80!black, align=center] at (1.75,1.25)
    {Commit after\\each step};

  \node[font=\scriptsize, text=WarmBlue!70] at (1.75,-0.8)
