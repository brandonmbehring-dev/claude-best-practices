...
and \code{.claude/settings.json} are committed to version control. Every team member gets the same
project context, the same permissions, and the same hooks.
...
\begin{minted}{text}
In PR #234, the author changed the caching strategy
in src/cache/redis_client.py.
What are the implications for cache invalidation?
Check if any callers depend on the old TTL behavior.
\end{minted}
